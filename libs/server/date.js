...
module.exports = {
    ...
    toMinutes: function toMinutes(span) {
        return !span ? 0 :
            (span.days || 0) * 1440 + (span.hours || 0) * 60 + (span.mins || 0);
    },
    getTTL: function(s, seconds) {//session wiil not expire within N ms
        var sec = parseInt(s || '0'),
            interval = (!!seconds ? sec * 1000 : sec) - Date.now();
        return {
            ms: interval,
